...
//	int sum() const { return a + b + c; }	// error : a를 사용
	int sum() const { return b + c; }		// OK
	void printbc() const { cout << b << "" << c << '\n'; }	// OK
};

class Drvd2 : protected BaseC {
	/*
	class BaseC의 데이터 멤버 a는 class Drvd2가 액세스 할 수 없음
...
class Drvd3 : private BaseC {
	/*
	class BaseC의 데이터 멤버 a는 class Drvd3가 액세스 할 수 없음
...
//	d1.b = 2;	// protected 멤버를 액세스 하므로 error
	d1.c = 3;	// public 멤버를 액세스 하므로 OK

	Drvd2 d2;
//	d1.a = 1;	// private 멤버를 액세스 하므로 error
//	d1.b = 2;	// protected로 취급되는 멤버를 액세스 하므로 error
//	d1.c = 3;	// protected로 취급되는 멤버를 액세스 하므로 error

	Drvd3 d3;
//	d1.a = 1;	// private 멤버를 액세스 하므로 error
//	d1.b = 2;	// private로 취급되는 멤버를 액세스 하므로 error
//	d1.c = 3;	// private로 취급되는 멤버를 액세스 하므로 error
